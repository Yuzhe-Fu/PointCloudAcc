...
| Array Parameter |
| 0         | 1-2               | 3-22       | 23-38        |
| Mode(1b)  | CCUSYA_CfgMod(2b)  | Ly_Num(20b)| B_Addr0(32b) |
| Layer Parameters |
| 0-2       | 3-34          | 35-66         | 67-82 | 83-94 | 95-106| 107-126   | 127-134       | 135-142           |
| OpCodeCON| DatAddr(32b)   | WgtAddr(32b)   | Nip(16b)| Chi   | Cho   |quant_scale| quant_shift   | quant_zero_point  | 
...
| Array Parameter |
| 0 | 1-2 | 3-22 | 23-38 |
| Mode: 1b, Run/Test | CCUSYA_CfgMod: 2b, 2x2, 4x1, 1x4 | Ly_Num: 20b | B_Addr0: 32b |
| Layer Parameters |
...
CCU是中央控制器，有专门的4bit IO来读指令集
    ...
    - 控制片内：用FSM，t每层输出一次配置，IDLE(芯片启动空状态）->RD_CFG(读取整个网络的参数配置）->LY_CFG（配置网络一层）->LY_WORK（网络一层工作）->FNH（整个网络计算完成）
    根据模块反馈出FPS、KNN、CONV、POL、FC等层的完成情况，各自取各自下一层的配置，每种层单独配置，Cfg不同信息归属不同种的层，所有种层的信息连续存到RAM，每个种层有个地址，根据取到的OpCode确认是否取到相应的配置，否则地址加1。
