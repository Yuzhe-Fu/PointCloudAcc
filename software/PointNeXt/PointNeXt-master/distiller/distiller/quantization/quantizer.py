...
class Quantizer(object):
    ...
    def _add_qbits_entry(self, module_name, module_type, qbits):
        # pdb.set_trace()
        # if module_type in [nn.Linear, nn.Embedding]:
        #     pdb.set_trace()
        if module_type not in [openpoints.models.layers.conv.Conv1d, openpoints.models.layers.conv.Conv2d, nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.Linear, nn.Embedding]:
    ...
    def prepare_model(self, dummy_input=None):
        ...
        logging.info('Preparing model for quantization using {0}'.format(self.__class__.__name__))
    ...
    def _pre_process_container(self, container, prefix=''):
        def replace_msg(module_name, modules=None):
            logging.info('Module ' + module_name)
            if modules:
                logging.info('\tReplacing: {}.{}'.format(modules[0].__module__, modules[0].__class__.__name__))
                logging.info('\tWith:      {}.{}'.format(modules[1].__module__, modules[1].__class__.__name__))
            else:
