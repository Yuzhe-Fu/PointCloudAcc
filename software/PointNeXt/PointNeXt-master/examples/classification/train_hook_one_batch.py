...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg, compression_scheduler, tflogger=None, pylogger=None):
    ...
    for idx, data in pbar:
        if idx == 0:
            ...
            logging.info(f'the size of weight in encoder.encoder[1][0].skipconv[0] is {model.encoder.encoder[1][0].skipconv[0].weight.size()}')
            logging.info(model.encoder.encoder[1][0].skipconv[0].weight)
            logging.info(f'the size of weight scale is {model.encoder.encoder[1][0].skipconv[0].weight_scale.size()}')
            logging.info(f'the weight scale is {model.encoder.encoder[1][0].skipconv[0].weight_scale}')
            logging.info(f'the size of zeropoint is {model.encoder.encoder[1][0].skipconv[0].weight_zero_point.size()}')
            logging.info(f'the zeropoint is {model.encoder.encoder[1][0].skipconv[0].weight_zero_point}')
