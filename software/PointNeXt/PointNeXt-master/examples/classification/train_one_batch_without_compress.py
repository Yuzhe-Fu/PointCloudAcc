...
def main(gpu, cfg, profile=False):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        model.encoder.encoder[0][0].convs[0][0].register_forward_hook(get_activation('0_conv1d'))
        model.encoder.encoder[1][0].convs[0][0].register_forward_hook(get_activation('1_conv2d'))
        model.encoder.encoder[2][0].convs[0][0].register_forward_hook(get_activation('2_conv2d'))
        model.encoder.encoder[3][0].convs[0][0].register_forward_hook(get_activation('3_conv2d'))
        model.encoder.encoder[4][0].convs[0][0].register_forward_hook(get_activation('4_conv2d'))
...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        if idx == 0:
            ...
            # torch.save(input_data['0_conv1d'], './data/TensorData/0_conv1d_input.pt')
            # logging.info('the input of encoder.encoder[0][0].convs[0][0] is')
            # logging.info(input_data['0_conv1d'].size())
            # logging.info(input_data['0_conv1d'])

            # torch.save(output_data['0_conv1d'], './data/TensorData/0_conv1d_output.pt')
            # logging.info('the output of encoder.encoder[0][0].convs[0][0] is')
            # logging.info(output_data['0_conv1d'].size())
            # logging.info(output_data['0_conv1d'])

            # torch.save(input_data['1_conv2d'], './data/TensorData/1_conv2d_input.pt')
            logging.info('the input of encoder.encoder[1][0].convs[0][0] is')
            logging.info(input_data['1_conv2d'].size())
            # pdb.set_trace()
            logging.info(input_data['1_conv2d'][0,0:3,0,0]) # (batch, channel, cp, np)
            logging.info(input_data['1_conv2d'][0,0:3,0,1])
            logging.info(input_data['1_conv2d'][0,0:3,1,0])
            logging.info(input_data['1_conv2d'][0,0:3,1,1])
            logging.info('**********************************')
            logging.info(input_data['2_conv2d'][0,0:3,0,0])
            logging.info(input_data['2_conv2d'][0,0:3,0,1])
            logging.info(input_data['2_conv2d'][0,0:3,1,0])
            logging.info(input_data['2_conv2d'][0,0:3,1,1])
            logging.info('**********************************')
            logging.info(input_data['3_conv2d'][0,0:3,0,0])
            logging.info(input_data['3_conv2d'][0,0:3,0,1])
            logging.info(input_data['3_conv2d'][0,0:3,1,0])
            logging.info(input_data['3_conv2d'][0,0:3,1,1])
            logging.info('**********************************')
            logging.info(input_data['4_conv2d'][0,0:3,0,0])
            logging.info(input_data['4_conv2d'][0,0:3,0,1])
            logging.info(input_data['4_conv2d'][0,0:3,1,0])
            logging.info(input_data['4_conv2d'][0,0:3,1,1])
            logging.info('**********************************')


            # logging.info(input_data['1_conv2d'][0,32:35,0,0])
            # logging.info(input_data['1_conv2d'][0,32:35,0,1])
            # logging.info(f'the size of weight in encoder.encoder[1][0].convs[0][0] is {model.encoder.encoder[1][0].convs[0][0].weight.size()}')
            # logging.info(model.encoder.encoder[1][0].convs[0][0].weight)
            # logging.info(f'the size of weight scale is {model.encoder.encoder[1][0].convs[0][0].weight_scale.size()}')
            # logging.info(f'the weight scale is {model.encoder.encoder[1][0].convs[0][0].weight_scale}')
            # logging.info(f'the size of zeropoint is {model.encoder.encoder[1][0].convs[0][0].weight_zero_point.size()}')
            # logging.info(f'the zeropoint is {model.encoder.encoder[1][0].convs[0][0].weight_zero_point}')
