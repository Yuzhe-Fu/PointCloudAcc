...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        # temp = [radius, nsample]
        # f_radius = open('./blue_data/radius_nsample.csv', 'a', newline='')
        # radius_write = csv.writer(f_radius)
        # radius_write.writerow(temp)

        # # the size of xyz is torch.Size([32, 1024, 3])
        # # the size of xyz is torch.Size([32, 512, 3])
        # # the size of xyz is torch.Size([32, 256, 3])
        # # the size of xyz is torch.Size([32, 128, 3])
        # f_xyz = open('./blue_data/xyz.csv', 'a')
        # xyz_write = csv.writer(f_xyz)
        # xyz_saved = np.reshape(xyz.cpu().numpy(),(-1,3))
        # xyz_write.writerow(xyz_saved)
        # pd.DataFrame(xyz_saved).to_csv('./blue_data/xyz_pd.csv',mode='a',header=None, index=None)

        # # the size of newxyz is torch.Size([32, 512, 3])
        # # the size of newxyz is torch.Size([32, 256, 3])
        # # the size of newxyz is torch.Size([32, 128, 3])
        # # the size of newxyz is torch.Size([32, 64, 3])
        # f_new_xyz = open('./blue_data/new_xyz.csv', 'a')
        # new_xyz_write = csv.writer(f_new_xyz)
        # new_xyz_saved = np.reshape(new_xyz.cpu().numpy(),(-1,3))
        # new_xyz_write.writerow(new_xyz_saved)
        # pd.DataFrame(new_xyz_saved).to_csv('./blue_data/new_xyz_pd.csv',mode='a',header=None, index=None)
        ...
        # # the size of idx is torch.Size([32, 512, 32])
        # # the size of idx is torch.Size([32, 256, 32])
        # # the size of idx is torch.Size([32, 128, 32])
        # # the size of idx is torch.Size([32, 64, 32])
        # f_idx = open('./blue_data/idx.csv', 'a')
        # idx_write = csv.writer(f_idx)
        # idx_saved = np.reshape(idx.cpu().numpy(),(-1,32))
        # idx_write.writerow(idx_saved)
        # pd.DataFrame(idx_saved).to_csv('./blue_data/idx_pd.csv',mode='a',header=None, index=None)
...
class QueryAndGroup(nn.Module):
    def __init__(self, radius: float, nsample: int,
                 relative_xyz=False, # changed here
                 normalize_dp=False,
